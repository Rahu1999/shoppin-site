...
export function ProductsClient({
  ...
  const [activeCategory, setActiveCategory] = useState<string>(urlCategory || initialCategorySlug);
  ...
  // The exact query the server component already fetched — reused so the
  // first client render doesn't re-fetch data already present in the HTML.
  ...
  const isInitialQuery = JSON.stringify(currentQuery) === initialQueryRef.current;

  const { data, isLoading, isError } = useQuery({
    queryKey: ['products', currentQuery],
    queryFn: () => apiGet<any>('/products', currentQuery),
    staleTime: 60000 * 5,
    initialData: isInitialQuery ? { items: initialProducts, meta: initialMeta } : undefined,
  });
                  ...
                  <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-100 rounded-xl shadow-lg transition-all z-40 py-2">
                    ...
                    {categories.map((cat) => (
                      <button
                        type="button"
                        key={cat.id}
                        onClick={() => { handleCategoryChange(cat.slug); setIsDropdownOpen(false); }}
                        className={`w-full text-left px-4 py-2.5 text-sm flex items-center justify-between hover:bg-gray-50 ${activeCategory === cat.slug ? 'font-semibold text-gray-900' : 'text-gray-600'}`}
                      >
                        {cat.name} {activeCategory === cat.slug && <Check className="h-3.5 w-3.5" />}
                      </button>
                    ))}
                  </div>
                </>
